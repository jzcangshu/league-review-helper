...
这一步的目的，是先用假数据跑通完整链路，理解项目需要什么目录结构。示例里已经有 Excel 名单、PDF 资料、审核结果 TXT，网页启动后会直接加载这些内容。
...
这一步的目的，是把原始材料整理成网页能读取的结构。你手头通常只有一个学校文件夹，里面有名单 Excel 和一批 PDF；准备脚本会根据 PDF 文件名创建空的审核结果 TXT，并核对名单和资料是否对得上。
...
方式 A：人工运行脚本，适合目录结构清楚、文件名规则稳定的情况。

```powershell
python codex-skills/league-review-prep/scripts/prepare_school_review.py --school-dir "某某中学" --update-web-sources
```

方式 B：使用本项目提供的 Codex skill，让 Codex 帮你判断哪个 Excel、哪个 PDF 目录、哪些姓名差异需要提醒。适合真实学校材料格式不统一、文件夹命名混乱、姓名可能有错别字的情况。
...
这一步发生在你已经通过网页把一个学校全部审核完之后。此时每个人的审核结论都保存在 `审核结果/<学校名>/<人名>_审核结果.txt`，但学校原始 Excel 还没有被更新。回写脚本会把这些 TXT 内容填入 Excel 的 `入团志愿书问题备注` 列。
